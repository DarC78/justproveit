...
const OUTPUT_DEFAULT = 'leads/HVAC-Contractors-UK-DataForSEO-Maps.csv';
...
function isRelevantMapsItem(item) {
  ...
  const relevant = /hvac|air conditioning|air con|a\/c|ventilation|heat pump|refrigeration|cooling|climate|mechanical services|duct|f-gas|fgas/.test(haystack);
  const excluded = /merchant|suppl|wholesal|training|course|college|school|parts|showroom|hardware|builders merchant|appliance store|electronics store|mobile phone|computer repair|car air conditioning|vehicle air conditioning|auto air conditioning/.test(haystack);
...
function rowFromMapsItem(item, locationName) {
  ...
  return {
    ...
    'business type': clean(item.category || 'hvac contractor'),
    'contact name': '',
