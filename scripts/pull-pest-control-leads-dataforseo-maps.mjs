...
const OUTPUT_DEFAULT = 'leads/Pest-Control-UK-DataForSEO-Maps.csv';
...
function isRelevantMapsItem(item) {
  ...
  const relevant = /pest control|exterminator|vermin|rat control|mice control|mouse control|wasp nest|bed bug|cockroach|bird control|proofing|fumigation/i.test(haystack);
  const excluded = /merchant|suppl|wholesal|training|course|college|school|parts|showroom|hardware|estate agent|property management|self storage|pet shop|veterinary|dog|cat|garden centre|cleaning|cleaner|roof|locksmith|electrician|plumb/.test(haystack);
...
function rowFromMapsItem(item, locationName) {
  ...
  return {
    ...
    'business type': clean(item.category || 'pest control'),
    'contact name': '',
