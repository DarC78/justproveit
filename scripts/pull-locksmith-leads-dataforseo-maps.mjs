...
const OUTPUT_DEFAULT = 'leads/Locksmiths-UK-DataForSEO-Maps.csv';
...
function isRelevantMapsItem(item) {
  ...
  const relevant = /locksmith|lock smith|auto locksmith|emergency locksmith|mobile locksmith|lock repair|lock replacement|key cutting|safe opening|access control|door lock|upvc lock|uPVC/i.test(haystack);
  const excluded = /merchant|suppl|wholesal|training|course|college|school|parts|showroom|hardware|builders merchant|appliance store|electronics store|mobile phone|computer repair|estate agent|property management|self storage|shoe repair only|timpson only/.test(haystack);
...
function rowFromMapsItem(item, locationName) {
  ...
  return {
    ...
    'business type': clean(item.category || 'locksmith'),
    'contact name': '',
