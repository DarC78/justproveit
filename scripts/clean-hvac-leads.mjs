...
const INPUTS = [
  'leads/HVAC-Contractors-UK-DataForSEO-Maps-National.csv'
];

const OUTPUT = 'leads/HVAC-Contractors-UK-Master.csv';
const CLEAN_NATIONAL = 'leads/HVAC-Contractors-UK-DataForSEO-Maps-National-Clean.csv';
...
function isRelevantHvac(row) {
  ...
  const hvacSignal = /hvac|air conditioning|air con|a\/c|ventilation|heat pump|refrigeration|cooling|climate|mechanical services|duct|f-gas|fgas|chiller/.test(haystack);
  const contentHvacSignal = /hvac|air conditioning|air con|a\/c|ventilation|heat pump|refrigeration|cooling|climate|mechanical|duct|f-gas|fgas|chiller/.test(contentHaystack);
  const plumbingOnlySignal = /plumb|boiler|gas engineer|gas service|heating engineer|heating service|central heating/.test(contentHaystack);
  const excluded = /charging station|auto electrician|auto electrics|auto electrical|car electrician|car electrics|mechanic|mobile vehicle|diagnostic|halfords|foodbank|hairdressing|kitchen fitter|cleaning|cleaners|mobile phone|appliance store|lighting centre|serviced offices|electric studios|parking charging|destination charging|car air conditioning|vehicle air conditioning|auto air conditioning|air con re gas|regas|re-gas|radiator repair|garage|autocentre|auto centre|autocare|autocooling|autoair| autos |dtauto|mot | mot$|tyre|tires|motor works|motor company|motors|fleetcare|starter motor|alternator|accident repair|clutch|brakes|volkswagen|skoda|cupra|air conditioning repair shop|air conditioning store|recruitment|job centre|manufacturer|wholesale|supplier|showroom|fireplace|stove/.test(haystack);

  return hvacSignal && !excluded && (!plumbingOnlySignal || contentHvacSignal);
}
...
function cleanRows(rows) {
  ...
  for (const row of rows) {
    ...
    if (!isRelevantHvac(row)) continue;
...
async function main() {
  ...
  const nationalRows = cleanRows(await readRows('leads/HVAC-Contractors-UK-DataForSEO-Maps-National.csv'));
