...
const OUTPUT_DEFAULT = 'leads/Roofing-Companies-UK-DataForSEO-Maps.csv';
...
function isRelevantMapsItem(item) {
  ...
  const relevant = /roofing|roofer|roof repair|flat roof|slate roof|tile roof|guttering|fascia|soffit|commercial roofing|roofline/i.test(haystack);
  const excluded = /merchant|suppl|wholesal|training|course|college|school|parts|showroom|hardware|builders merchant|estate agent|property management|self storage|cleaning|cleaner|window cleaner|pest|locksmith|electrician|plumb|hvac|solar panel only/.test(haystack);
...
function rowFromMapsItem(item, locationName) {
  ...
  return {
    ...
    'business type': clean(item.category || 'roofing company'),
    'contact name': '',
