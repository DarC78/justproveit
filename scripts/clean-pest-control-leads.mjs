...
const INPUTS = [
  'leads/Pest-Control-UK-DataForSEO-Maps-National.csv',
  'leads/Pest-Control-UK-DataForSEO-Maps-National-2.csv',
  'leads/Pest-Control-UK-DataForSEO-Maps-National-3.csv'
];

const OUTPUT = 'leads/Pest-Control-UK-Master.csv';
const CLEAN_NATIONAL = 'leads/Pest-Control-UK-DataForSEO-Maps-National-Clean.csv';
...
function isRelevantTrade(row) {
  ...
  const tradeSignal = /pest control|exterminator|vermin|rat control|mice control|mouse control|wasp nest|bed bug|cockroach|bird control|proofing|fumigation/.test(haystack);
  const excluded = /plumb|boiler|gas engineer|heating engineer|hvac|air conditioning|electrician|charging station|mechanic|mot | mot$|tyre|garage|autocentre|auto centre|foodbank|hairdressing|laundry|mobile phone|appliance store|lighting centre|serviced offices|estate agent|property management|self storage|training|course|college|school|manufacturer|wholesale|supplier|showroom|roofing|roofer|locksmith|window cleaner|cleaning company|pet shop|veterinary|dog groom|cat groom|florida|boca raton|yamcha\.cloud/.test(haystack);

  return tradeSignal && !excluded;
}
...
function cleanRows(rows) {
  ...
  for (const row of rows) {
    ...
    if (!isRelevantTrade(row)) continue;
...
async function main() {
  ...
  const nationalRows = cleanRows(await readRows(INPUTS[0]));
