...
const OUTPUT_DEFAULT = 'leads/Window-Cleaners-UK-DataForSEO-Maps.csv';
...
function isRelevantMapsItem(item) {
  ...
  const relevant = /window clean|window cleaner|cleaning service|commercial window|domestic window|reach and wash|gutter clean|conservatory clean|fascia|soffit|high level cleaning/i.test(haystack);
  const excluded = /merchant|suppl|wholesal|training|course|college|school|parts|showroom|hardware|estate agent|property management|self storage|car wash|car detailing|laundry|dry clean|roofing|roofer|pest|locksmith|electrician|plumb|hvac/.test(haystack);
...
function rowFromMapsItem(item, locationName) {
  ...
  return {
    ...
    'business type': clean(item.category || 'window cleaner'),
    'contact name': '',
