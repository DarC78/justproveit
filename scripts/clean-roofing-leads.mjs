...
const INPUTS = [
  'leads/Roofing-Companies-UK-DataForSEO-Maps-National.csv',
  'leads/Roofing-Companies-UK-DataForSEO-Maps-National-2.csv',
  'leads/Roofing-Companies-UK-DataForSEO-Maps-National-3.csv'
];

const OUTPUT = 'leads/Roofing-Companies-UK-Master.csv';
const CLEAN_NATIONAL = 'leads/Roofing-Companies-UK-DataForSEO-Maps-National-Clean.csv';
...
function isRelevantTrade(row) {
  ...
  const tradeSignal = /roofing|roofer|roof repair|flat roof|slate roof|tile roof|guttering|fascia|soffit|commercial roofing|roofline/.test(haystack);
  const excluded = /plumb|boiler|gas engineer|heating engineer|hvac|air conditioning|electrician|charging station|mechanic|mot | mot$|tyre|garage|autocentre|auto centre|foodbank|hairdressing|laundry|mobile phone|appliance store|lighting centre|serviced offices|estate agent|property management|self storage|training|course|college|school|manufacturer|wholesale|supplier|showroom|locksmith|pest control|window cleaner|cleaning company|solar panel only|florida|boca raton|yamcha\.cloud/.test(haystack);
