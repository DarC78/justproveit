...
const INPUTS = [
  'leads/Local-Electricians-UK-DataForSEO-Maps-National.csv'
];

const OUTPUT = 'leads/Local-Electricians-UK-Master.csv';
const CLEAN_NATIONAL = 'leads/Local-Electricians-UK-DataForSEO-Maps-National-Clean.csv';
...
function isRelevantElectrician(row) {
  const haystack = [
    row['business name'],
    row['business type'],
    row.website,
  ...
  const relevant = /electric|electrical|electrics|sparky|eicr|pat test|rewir|fusebox|fuse box|consumer unit|lighting|ev charger|ev charging|solar|renewable|power|data|fire|security|alarm|cctv/.test(haystack);
  const excluded = /charging station|auto electrician|auto electrics|auto electrical|car electrician|car electrics|mechanic|mobile vehicle|diagnostic|halfords|plumber|plumbing|boiler|gas safety|hairdressing|kitchen fitter|cleaning|foodbank|mobile phone|appliance store|lighting centre|serviced offices|electric studios|parking charging|destination charging/.test(haystack);
...
function cleanRows(rows) {
  ...
  for (const row of rows) {
    ...
    if (!isRelevantElectrician(row)) continue;
...
async function main() {
  ...
  const nationalRows = cleanRows(await readRows('leads/Local-Electricians-UK-DataForSEO-Maps-National.csv'));
