...
const OUTPUT_DEFAULT = 'leads/Small-Plumbing-Companies-UK-DataForSEO-Maps.csv';
...
async function queryMaps({ login, password, keyword, locationName, depth }) {
  ...
  if (!response.ok || payload.status_code !== 20000 || payload.tasks?.[0]?.status_code !== 20000) {
    throw new Error(`DataForSEO Maps request failed: ${payload.status_code || response.status} ${payload.status_message || response.statusText}`);
  }
...
function isRelevantMapsItem(item) {
  ...
  const relevant = /plumb|boiler|heating|gas engineer|bathroom|drainage|drain/.test(haystack);
  const excluded = /merchant|suppl|wholesal|training|course|college|school|parts|showroom|hardware|builders merchant/.test(haystack);
...
function rowFromMapsItem(item, locationName) {
  ...
  return {
    ...
    'business type': clean(item.category || 'plumbing'),
    'contact name': '',
