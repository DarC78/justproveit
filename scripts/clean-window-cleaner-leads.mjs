...
const INPUTS = [
  'leads/Window-Cleaners-UK-DataForSEO-Maps-National.csv',
  'leads/Window-Cleaners-UK-DataForSEO-Maps-National-2.csv',
  'leads/Window-Cleaners-UK-DataForSEO-Maps-National-3.csv'
];

const OUTPUT = 'leads/Window-Cleaners-UK-Master.csv';
const CLEAN_NATIONAL = 'leads/Window-Cleaners-UK-DataForSEO-Maps-National-Clean.csv';
...
function isRelevantTrade(row) {
  ...
  const tradeSignal = /window clean|window cleaner|commercial window|domestic window|reach and wash|gutter clean|conservatory clean|fascia|soffit|high level cleaning/.test(haystack);
  const excluded = /plumb|boiler|gas engineer|heating engineer|hvac|air conditioning|electrician|charging station|mechanic|mot | mot$|tyre|garage|autocentre|auto centre|foodbank|hairdressing|laundry|mobile phone|appliance store|lighting centre|serviced offices|estate agent|property management|self storage|training|course|college|school|manufacturer|wholesale|supplier|showroom|locksmith|pest control|roofing|roofer|dry cleaner|car wash|car detailing|florida|boca raton|yamcha\.cloud/.test(haystack);
